...
console.log("--------------------------------------------------"); // http://stackoverflow.com/questions/3011600/clear-javascript-console-in-google-chrome

// books and links must be set up before anything else
var books = setupBooks();
var links = setupLinks();

// Create layers for each type of link
...
var processAllLinks = false;   // Should all links be processed? Set to "false" during testing
var debugOn = !processAllLinks;  // turn on debugging if we aren't processing all links

var trackMousePointer = false; // Show a circle wherever the mouse pointer goes
...
layerBooks.visible = false; // turn off visible until the books are processed...
...
layerBooks.visible = true; // ...now re-enable book arc visibility

var bk1, bk2;

// Turn off visible until the links are processed...
...
function addLink(aDeg, bDeg, bookName, linkType) {
	if ((linkType == "q" && showQuotations) || (linkType == "a" && showAllusions) || (linkType == "p" && showPossibleAllusions)) {
		...
		// simpleDebugLog(bookName + ": aDeg/startAngle: " + toDeg(aDeg) + "; bDeg/toAngle: " + toDeg(bDeg) + "; linkType: " + linkType + "; through1: " + toDeg(through1));
...
function getLinkThroughPoint(r, a1, a2) {
	...
	var maxAngle = Math.max(d1, d2);
	var minAngle = Math.min(d1, d2);
	var to360Max = 360 - maxAngle; // Move the largest angle to 0
	var minAngleAdjustment = minAngle + to360Max;
	// simpleDebugLog("minAngleAdjustment: " + minAngleAdjustment);
	...
	if (minAngleAdjustment < 15) {
		...
		midRadius = (5*r)/10;
		// simpleDebugLog("midRadius set to 5r/10");
...
function setupLinks() {
	...
	} else { // Only push a subset (during testing)
		simpleDebugLog("setting up short list of links");
		links = setupShortLinkList();
	}
	return(links);
}
