...
for (var bkNum = 0; bkNum < books.length; bkNum++) {
	...
	drawBookArc(r, toRad(lastStart), toRad( lastStart + bkSize ), getBookArcColor(books[bkNum]), books[bkNum].bkName);
...
function drawBookArc(r, a1, a2, color, bookName) {
	var startA, endB;
	var startB, endB;
	var pLine, pColor;
	...
	if (toDeg(a1) < 90 || toDeg(a1) > 270) { // rotate the label properly
		text.rotate(toDeg(Math.min(a2,a1)+((Math.max(a2,a1)-Math.min(a2,a1))/2)));
	} else {
		text.rotate(180+toDeg(Math.min(a2,a1)+((Math.max(a2,a1)-Math.min(a2,a1))/2)));
		var bnds = text.bounds;
		// console.log("text.bounds.width for " + bookName + " = " + bnds.width);
		// TODO fix the hard-coded 25 value. the "width" value isn't being set properly
